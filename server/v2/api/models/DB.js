...
class StoreData {
  ...
  async createRecord(data) {
    const textQuery = 'SELECT * FROM challenge.dataDB WHERE recordid in(SELECT MAX(recordid) FROM challenge.dataDB);';
  ...
  async fetchOneUser(Id) {
    const text = 'SELECT * FROM challenge.userDB WHERE id = $1';
    const values = [Id];
    const { rows } = await pool.query(text, values);
    return rows;
  }
